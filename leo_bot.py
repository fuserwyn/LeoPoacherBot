...
async def handle_chat_member_updated(event: types.ChatMemberUpdated):
    ...
    if event.new_chat_member.status == ChatMemberStatus.MEMBER:
        ...
        try:
            welcome_message = f"👋 **Добро пожаловать, @{username}!**\n\n🤖 **Я LeoPoacherBot - ваш тренер!**\n\n⏰ **Таймер уже запущен!**\n• У вас есть **2 минуты** чтобы отправить `#training_done`\n• Через **1 минуту** вы получите предупреждение\n• Через **2 минуты** без отчета - удаление из чата\n\n💪 **Отправьте `#training_done` прямо сейчас!**"
        ...
        # Планируем удаление через 2 минуты для нового участника
        logging.info(f"⏰ ЗАПУСК ТАЙМЕРА для нового участника {user_id} (@{username})")
        warning_task = asyncio.create_task(schedule_user_warning(user_id, chat_id, username, 60))  # 60 секунд = 1 минута
        removal_task = asyncio.create_task(schedule_user_removal(user_id, chat_id, 120))  # 120 секунд = 2 минуты
        scheduled_removals[user_id] = {"warning": warning_task, "removal": removal_task}
        logging.info(f"✅ Таймер запущен для нового участника {user_id} (@{username}) - предупреждение через 60 сек, удаление через 120 сек")
...
• При получении `#training_done` таймер перезапускается на 2 минуты
• Через 1 минуту без `#training_done` - предупреждение
• Через 2 минуты без `#training_done` - удаление из чата
...
"""
    ...
    if user_id not in scheduled_removals:
        ...
        warning_task = asyncio.create_task(schedule_user_warning(user_id, chat_id, username, 60))
        removal_task = asyncio.create_task(schedule_user_removal(user_id, chat_id, 120))
        scheduled_removals[user_id] = {"warning": warning_task, "removal": removal_task}
    ...
    if has_training_done:
        ...
        # Запускаем новый персональный таймер на 2 минуты после #training_done
        username = msg.from_user.username or msg.from_user.first_name
        timer_start_time = current_time.isoformat()
        logging.info(f"ЗАПУСК НОВОГО ТАЙМЕРА: После #training_done планируем удаление пользователя {user_id} (@{username}) через 2 минуты")
        warning_task = asyncio.create_task(schedule_user_warning(user_id, chat_id, username, 60, timer_start_time))  # 60 секунд = 1 минута
        removal_task = asyncio.create_task(schedule_user_removal(user_id, chat_id, 120, timer_start_time))  # 120 секунд = 2 минуты
        scheduled_removals[user_id] = {"warning": warning_task, "removal": removal_task}
        logging.info(f"Новый персональный таймер запущен для пользователя {user_id} - предупреждение через 60 сек, удаление через 120 сек")
    else:
