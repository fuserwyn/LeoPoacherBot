...
func (b *Bot) generateLeoTrainingFeedEncouragement(
	...
	sickHint := ""
	if wasOnSickLeave {
		sickHint = "пользователь недавно снимал больничный — можно тёплую нотку «рад что снова в строю», без морали."
	}

	question := strings.TrimSpace(b.config.Prompts.TrainingFeedEncouragement)
	if question == "" {
		question = "Напиши лаконичный живой комментарий Лео в тред ленты после тренировки. Plain text."
	}
	question += `
...
СТРУКТУРА ОТВЕТА:
1) 1–4 предложения по сути отчёта и подсказок ниже.
2) Пустая строка.
3) Короткий финал отдельной строкой («Держи ритм…») — см. подсказку по полу.`
...
func (b *Bot) LeoBanterReplyToUserTrainingFeedThread(
	...
	qb := strings.Builder{}
	qb.WriteString("Ты Лео, тренер Fat Leopard. Пользователь ответил на ТВОЁ сообщение в комментариях под его отчётом о тренировке в ленте стаи (мини-апп).\n\n")
	qb.WriteString("Тон v1.3: дерзкий, прямой, без хэштегов, без «съем», без садизма, без ремарок в *звёздочках*.\n\n")
