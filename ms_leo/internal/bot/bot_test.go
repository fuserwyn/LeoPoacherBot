...
func TestIsPackCommandHashtag(t *testing.T) {
	if !isPackCommandHashtag("#training_done") {
		t.Fatal("bare #training_done")
	}
	if !isPackCommandHashtag("Сегодня зал #TRAINING_DONE") {
		t.Fatal("hashtag in sentence")
	}
	if !isPackCommandHashtag("бег, 15 мин, инт. 3/5") {
		t.Fatal("miniapp report line")
	}
	if !isPackCommandHashtag("#sick_leave простуда") {
		t.Fatal("sick leave")
	}
	if !isPackCommandHashtag("#healthy") {
		t.Fatal("healthy")
	}
	if isPackCommandHashtag("просто потренировался") {
		t.Fatal("plain text must stay silent in group")
	}
	if isPackCommandHashtag("") {
		t.Fatal("empty")
	}
}

func TestMessageTextOrCaption(t *testing.T) {
	if got := messageTextOrCaption(&tgbotapi.Message{Caption: "#training_done"}); got != "#training_done" {
		t.Fatalf("caption: %q", got)
	}
	if got := messageTextOrCaption(&tgbotapi.Message{Text: "#training_done", Caption: "photo"}); got != "#training_done" {
