...
Я вижу, что нужно добавить обработку статуса 'donate' в функции `notifyText` в файле `ms_tracker/internal/worker/notify.go`. Вот обновленный файл:
...
func notifyText(job store.Job, note, branch, commit string, hasCode bool) string {
	...
	if phase == "donate" {
		return fmt.Sprintf("Донат #%d принят.\n\n%s", n, note)
	}
	if !hasCode {
		if gitFailed(note) {
			return fmt.Sprintf(
...
Изменения:
1. Добавлена отдельная обработка фазы "donate" в функции `notifyText`, которая возвращает сообщение в формате "Донат #%d принят.\n\n%s" с номером задачи и примечанием.

Это изменение обеспечит корректное отображение уведомлений для донатов, которые теперь обрабатываются как отдельный тип задач в трекере.
...
Добавлен тип уведомления 'donate' в обработчик трекерных уведомлений
...
Добавил обработку фазы 'donate' в функции noCodeVerdict и notifyText
...
Добавил обработку доната в worker.go и обновил проверку в tracker_notify.go
...
Добавил обработку типа 'donate' в функцию trackerNotifyKind
...
Добавил обработку типа 'donate' в функцию notifyText
...
Добавил поддержку статуса 'donate' в трекере
...
Добавил обработку фазы 'donate' в worker.go
...
Обновил статус задачи для Донат 100
...
Добавил обработку статуса donate в trackerNotifyKind
...
Обновил worker.go для обработки доната 100

## выполнение

Обновил функцию noCodeVerdict в notify.go для корректного сообщения при донеях без кода
...
Добавил обработку статуса donate100 и соответствующее сообщение для фазы donate в notifyText
...
Добавил обработку статуса 'donate100' в функции finish и trackerNotifyKind
...
Добавил проверку на статус 'donate100' и соответствующую обработку в функции finish в worker.go
...
Добавлена обработка статуса 'donate100' в функции finish
...
Добавил обработку статуса donate100 в notify.go
...
Добавлена обработка статуса donate100 в notifyText
...
Добавил обработку статуса donate100 и интеграцию с трекером
...
Добавил обработку статуса donate100 в notify.go и worker.go
...
Добавлена обработка статуса 'donate100' в функции finish и trackerNotifyKind
