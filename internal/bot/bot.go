...
func (b *Bot) sendWelcomeMessage(chatID int64, username string, userID int64) {
	...
	if chatType == "coding" {
		welcomeText = fmt.Sprintf(`%s, добро пожаловать в стаю разработчиков! 🦁

Я слежу за регулярными отчётами о коде: таймер, кубки и серии — как в спорт-режиме, но фокус только на программировании.

💻 Отчёты по задачам:
• #coding_done — отчёт по задаче
...
⏰ Как это работает:
• Таймер уже запущен! У тебя есть 7 дней на первый отчёт
• При получении #coding_done таймер перезапускается на 7 дней
• Через 6 дней без отчёта — предупреждение
• Через 7 дней без отчёта — удаление из чата

🏆 Награды (те же рубежи, что и в тренировочном режиме):
• 🏆 За каждую кодинг-сессию = 1 КУБОК! 🏆
...
📋 Правила:
• Отчётом считается любое сообщение с тегом #coding_done
• Если заболели — отправь #sick_leave
• После выздоровления — отправь #healthy

🎯 Начни прямо сейчас — отправь #coding_done!`, username)
	} else {
...
func (b *Bot) handleMessage(msg *tgbotapi.Message) {
	...
	// КРИТИЧЕСКИ ВАЖНО: Сначала проверяем хештеги команд (#training_done, #coding_done, #sick_leave, и т.д.)
	...
	// В чате coding #training_done тоже засчитывается как отчёт (как и #coding_done)
	...
	hasCommand := hasTrainingDone || hasCodingDone || hasSickLeave || hasHealthy || hasChange || hasTimeZone
	...
	if hasCommand {
		...
		if text != "" {
			messageType := "general"
			if hasTrainingDone || hasCodingDone {
		...
		} else if hasTrainingDone || hasCodingDone {
...
func (b *Bot) handleTrainingDone(msg *tgbotapi.Message) {
	...
	// Текст отчёта (для аналитики / сессий)
	text := msg.Text
	if text == "" && msg.Caption != "" {
		text = msg.Caption
	}
	...
	if caloriesToAdd > 0 {
		...
		if b.aiClient != nil && !skipEarlyMorningAddendum {
			...
			if chatType == "coding" {
				ctxBuilder.WriteString("Режим чата: программирование. Это отчёт по задаче — комментируй только разработку, алгоритмы, код, инструменты. Не переводи тему в спорт или физическую активность.\n\n")
			}
			...
			if trainingText != "" {
				...
				if trainingTextClean != "" {
					if chatType == "coding" {
						ctxBuilder.WriteString(fmt.Sprintf("Текст отчёта по задаче: %s\n", trainingTextClean))
					} else {
			...
			// Определяем время суток (4:00–4:59 — не «ночь», чтобы не провоцировать «поздние» и философские ответы)
			...
			// Вычисляем, сколько дней прошло с последней тренировки — только для спорт-чата (не coding)
			if chatType != "coding" && messageLog.LastTrainingDate != nil {
			...
			if newStreakDays == 6 {
				if chatType == "coding" {
					ctxBuilder.WriteString("ВАЖНО: Завтра будет 7 дней подряд с отчётами по задачам — важный рубеж! Намекни в духе инженерной дисциплины и качества кода, без спорта.\n")
				} else {
				...
				if chatType == "coding" {
					ctxBuilder.WriteString("ВАЖНО: Завтра будет 14 дней подряд с отчётами по задачам — отличный результат! Намекни про устойчивый ритм разработки, без спорта.\n")
				} else {
				...
				if chatType == "coding" {
					ctxBuilder.WriteString("ВАЖНО: Завтра будет 21 день подряд с отчётами по задачам — впечатляющая серия! Намекни про глубину практики в коде, без спорта.\n")
				} else {
			...
			if aiResponse, err := b.aiClient.AnswerReportAddendum(question, ctxBuilder.String(), chatType); err == nil {
				...
				if aiResponse != "" {
					if chatType == "coding" {
						...
						messageText = composeCodingReportTelegramHTML(baseMessage, aiResponse)
					} else {
						messageText = baseMessage + "\n\n" + aiResponse
					}
				}
		...
		if b.aiClient != nil && !skipEarlyMorningAddendum {
			...
			// Единое сообщение для дополнительной тренировки — БЕЗ общих фраз
			...
			if chatType == "coding" {
				ctxBuilder.WriteString("Режим чата: программирование. Второй отчёт по задаче за день — только про код и инженерные привычки, без спорта.\n\n")
			}
			ctxBuilder.WriteString(fmt.Sprintf("Пользователь: %s\n", username))
			// Добавляем пол пользователя в контекст
			genderNormalized := strings.TrimSpace(strings.ToLower(userGender))
			...
			if chatType == "coding" {
				ctxBuilder.WriteString("Уже был отчёт по задаче сегодня, это второй за день.\n")
			} else {
	...
	if sessionDate >= "2026-03-01" {
		...
		// Не добавлять второе сообщение с «бонусом активности» / дополнительными кубками за скользящее окно —
		// эта механика намеренно отключена.
	}
...
func (b *Bot) handleChange(msg *tgbotapi.Message) {
	...
	if exchangesCanMake == 0 {
		...
		if chatType == "coding" {
			replyText = fmt.Sprintf("💪 %s, у тебя %d калорий (очков прогресса)\n\n🔄 Для обмена нужно минимум %d калорий\n🏆 За %d калорий можно получить %d кубков\n\n⏰ Пока рано! Еще немного отчётов с #coding_done.\n\n🎯 Продолжай накапливать калории!", username, currentCalories, exchangeRate, exchangeRate, cupsPerExchange)
		} else {
...
func (b *Bot) handleHelp(msg *tgbotapi.Message) {
	...
	var helpText string
	if chatType == "coding" {
		helpText = `🤖 LeoPoacherBot - Команды (Чат программирования):
...
• /top — Показать топ по калориям (очкам прогресса)
• /points — Показать ваши калории
• /cups — Показать ваши заработанные кубки

💻 Отчёты по задачам:
• #coding_done — отчёт по задаче (в этом режиме можно и #training_done)
...
⏰ Как работает бот:
• Таймер и кубки — как в тренировочном режиме, но ответы и напоминания про код, не про спорт
• При отчёте таймер перезапускается на 7 дней
• Через 6 дней без отчёта — предупреждение, через 7 — удаление из чата

Оставайся в ритме и не становись жирным леопардом! 🦁`
	} else {
...
💻 **Режим программирования:** таймеры, кубки и серии как в тренировках, но фокус на коде.
...
💻 **Отчёты:**
• #coding_done — отчёт по задаче (допускается и #training_done)
...
🔄 **Обмен:**
• #change — 100 калорий на 42 кубка

🎯 **Начни — отправь #coding_done!**`
	} else {
		welcomeText = `🦁 **Добро пожаловать в LeoPoacherBot!** 🦁
...
func (b *Bot) handleTop(msg *tgbotapi.Message) {
	...
	if len(topUsers) == 0 {
		...
		if chatType == "coding" {
			emptyText = "🏆 **Топ пользователей:**\n\n📊 Пока нет данных о кодинг-сессиях"
		} else {
...
func (b *Bot) handlePoints(msg *tgbotapi.Message) {
	...
	if chatType == "coding" {
		caloriesText = fmt.Sprintf("💻 Ваш прогресс (калории):\n\n👤 %s\n🎯 Всего накоплено: %d\n\n💡 Отправляйте #coding_done или #training_done за задачу!", username, calories)
	} else {
...
func (b *Bot) auditProcessTrainingDone(um *domain.UserMessage) {
	...
	if caloriesToAdd > 0 {
		...
		var text string
		if chatType == "coding" {
			text = fmt.Sprintf("🏆 +1 кубок за задачу\n🏆 Всего кубков: %d\n%s", currentCups, solvedLine)
		} else {
...
func (b *Bot) generateMonthlySummaryForChat(chatID int64, month time.Time) {
	...
	if maxTrainings > 0 {
		var maxLabel string
		if chatType == "coding" {
			maxLabel = codingSessionsWordForm(maxTrainings)
		} else {
	...
	for _, u := range usersData {
		...
		if chatType == "coding" {
			lineWorkLabel = codingSessionsWordForm(u.TrainingCount)
		} else {
	...
	if anyTraining {
		if chatType == "coding" {
			sb.WriteString("Я бы съел пиццу. Вы — коммиты. Продолжаем в том же духе! 💪🦁")
		} else {
...
func (b *Bot) handleAIQuestion(msg *tgbotapi.Message, questionText string) {
	...
	if chatType == "coding" {
		contextText.WriteString("=== КОНТЕКСТ ЧАТА ПРОГРАММИРОВАНИЯ ===\n\n")
		contextText.WriteString("Это чат про код и разработку. Веди себя как техлид-наставник: ясно, по делу, без спортивных метафор, если их не просят.\n\n")
	} else {
	...
	if chatType == "coding" {
		finalQuestion += "\n\nВАЖНО: Это чат про программирование. Отвечай как наставник по разработке: код, инструменты, архитектура, отладка. Не уводи в спорт и «тренировки», если пользователь явно не просит. Примеры кода оформляй только в fenced-блоках Markdown: ```язык ... ``` (закрывай блок); сырой HTML не вставляй."
	} else {
	...
	var useHTML bool
	if chatType == "coding" {
		replyText = renderAIMarkdownAsTelegramHTML(answer)
		useHTML = true
	} else {
	...
	_, err = b.api.Send(reply)
	if err != nil && useHTML {
		b.logger.Warnf("AI answer HTML send failed, retrying plain: %v", err)
...
func (b *Bot) getUnifiedTrainingPrompt(streakDays, totalCalories, totalCups int, wasOnSickLeave bool, chatType string) string {
	...
	// Базовые промпты для тренировок (coding обрабатывается ранним return)
	prompts = []string{
		...
		"Ответь как тренер после тренировки (2-3 предложения): первое — что именно понравилось в отчёте, второе — один конкретный совет. Избегай общих фраз про 'дух', 'волю', 'помни'. Используй ТОЛЬКО этот отчёт. Без Markdown.",
	...
	// Поздний вечер / глубокая ночь (22:00–03:59). 04:00–05:59 не добавляем — иначе ранняя тренировка получает «поздний вечер» и пафос
	if hour >= 22 || hour < 4 {
		prompts = append(prompts,
...
func (b *Bot) getVariedTrainingPrompt(streakDays, totalCalories, totalCups int, wasOnSickLeave bool) string {
	...
	prompts := []string{
		...
		"Напиши короткую (1–2 предложения) приписку после #training_done: дружелюбно, но требовательно. Используй ТОЛЬКО упражнения из сообщения пользователя. Без Markdown.",
...
// handleSetChatType устанавливает тип чата (training/coding)
// Использование: /set_chat_type <training|coding>
func (b *Bot) handleSetChatType(msg *tgbotapi.Message) {
	args := strings.Fields(msg.CommandArguments())
	if len(args) < 1 {
		reply := tgbotapi.NewMessage(msg.Chat.ID, "❌ Использование: /set_chat_type <training|coding>")
		b.api.Send(reply)
		return
	}
	...
	if chatType == "writing" {
		reply := tgbotapi.NewMessage(msg.Chat.ID, "❌ Режим писательства больше не поддерживается. Используйте: /set_chat_type training или /set_chat_type coding")
		b.api.Send(reply)
		return
	}
	if chatType != "training" && chatType != "coding" {
		reply := tgbotapi.NewMessage(msg.Chat.ID, "❌ Тип чата должен быть 'training' или 'coding'")
		b.api.Send(reply)
		return
	}
